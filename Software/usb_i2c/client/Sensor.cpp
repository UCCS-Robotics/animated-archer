...
Sensor::Sensor(QObject *p) : QObject(p)
{
	mEditor = new QTextEdit;
	mEditor->setWindowTitle("Sensor Data");
	mEditor->setReadOnly(true);
	mEditor->show();

	connect(&mTimer, SIGNAL(timeout()), this, SLOT(updateSensor()));
	...
	if(i2cReadSingle(TSL2561_ADDR_FLOAT) & 0x0A)
		mEditor->insertPlainText("Found TSL2561\n");
	else
		mEditor->insertPlainText("Missing TSL2561\n");
...
void Sensor::updateSensor()
{
	...
	mEditor->insertPlainText(tr("Light: %1\n").arg(visible));
...
void Sensor::write8(uint8_t reg, uint8_t value)
{
